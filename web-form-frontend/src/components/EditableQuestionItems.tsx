...
import { NewQuestionItem, ExistingQuestionItem } from '../interface/Question';
...
const EditableQuestionItems: React.FC<EditableQuestionItemsProps> = (props) => {
  ...
  const restoreQuestionItem: Function =
    props.restoreQuestionItem !== undefined
      ...
      : () => {};
  const items: (NewQuestionItem | ExistingQuestionItem)[] = props.items;
  ...
  return (
    <>
      ...
      <FormGroup sx={{ maxWidth: '50%' }}>
        {items.map(
          (item: NewQuestionItem | ExistingQuestionItem, i: number) => {
